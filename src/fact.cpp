...
template <>
void mulmod(unsigned long long& a, unsigned long long b,
...
void mulmod(long long& a, long long b, const long long& n)
{
    ...
    mulmod<unsigned long long> (aa, (unsigned long long)b,
...
const std::vector<T>& get_primes(const T& b, T2& pi)
{
    ...
    if (counted_b >= b)
        pi = T2(std::upper_bound(primes.begin(), primes.end(),
                                 b) - primes.begin());
    else
    {
        if (counted_b == 0)
        ...
        for (T cur = first; cur <= b; ++++cur)
        {
            ...
            for (typename std::vector<T>::const_iterator iter = primes.begin(),
                    end = primes.end();
                    iter != end; ++iter)
            {
                const T& div = *iter;
...
T2 prime_div_trivial(const T& n, T2 m)
{
    ...
    for (typename std::vector<T2>::const_iterator iter = primes.begin(),
            end = primes.end();
            iter != end; ++iter)
    {
        const T2& div = *iter;
...
T pollard_p_1(T n)
{
    ...
    for (int j = 0; j < 10; j++)
    {
        ...
        for (size_t i = 0; i < sizeof q / sizeof q[0]; i++)
        {
            ...
            T aa = powmod(a, powmod((T)qq, e, n), n);
...
T pollard_bent(T n, unsigned iterations_count = 19)
{
    ...
    for (unsigned iteration = 0, series_len = 1; iteration < iterations_count;
