...
#endif
#include "api.h"
unsigned rsax(unsigned char * from, int from_len, unsigned char * to,
              int to_len, BIGNUM * N, BIGNUM * E)
{
    BIGNUM x, y;
    BN_init(&x);
    BN_init(&y);
...
void rsa(unsigned char * from, size_t from_size, unsigned char * to,
         size_t to_size)
{
...
int pow_mod(unsigned char * y, unsigned char * g, size_t g_s,
